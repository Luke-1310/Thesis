...
def show_menu(screen, font, map_options, selected_map_name=None):
    
    #Bottoni
    buttons = [
        {"text": "1. Allenare l'agente", "action": "train"},
        {"text": "2. Mostrare risultati", "action": "show"},
        {"text": "3. Selezione della mappa", "action": "select_map"},
        {"text": "4. Esci", "action": "exit"}
    ]
    ...
    # Titolo centrato
    title = font.render("Schermata iniziale", True, (0, 0, 0))
    ...
    # Mostra il nome della mappa selezionata
    if selected_map_name:
        messaggio = font.render(f"Mappa selezionata: {selected_map_name}", True, (0, 0, 0))
        screen.blit(messaggio, (screen.get_width() // 2 - messaggio.get_width() // 2, y + 20))
...
#4 funzione che centra il testo, utile per stampare dei messaggi come "Vuoi salvare la Q-table"
def draw_text_centered(screen, text, y, font, color=(0, 0, 0)):
    text_surface = font.render(text, True, color)
    x = (screen.get_width() - text_surface.get_width()) // 2
    screen.blit(text_surface, (x, y))

#7 Selezione della mappa
def show_map_selector(screen, font, map_options):
    screen.fill((255, 255, 255))
    title = font.render("Seleziona una mappa", True, (0, 0, 0))
    screen.blit(title, (screen.get_width() // 2 - title.get_width() // 2, 50))

    map_buttons = []
    y = 150
    for label in map_options.keys():
        rect = pygame.Rect(screen.get_width() // 2 - 150, y, 300, 50)
        pygame.draw.rect(screen, (0, 128, 0), rect)
        text_surface = font.render(label, True, (255, 255, 255))
        screen.blit(text_surface, (rect.x + 20, rect.y + 10))
        map_buttons.append((rect, label))
        y += 80
...
def main():
    ...
    map_options = {
        "Mappa A": "city_map.png",
        "Mappa B": "NULL.png",
    }

    in_map_selector = False

    while running:
        if in_map_selector:
            map_buttons = show_map_selector(screen, font, map_options)
        else:
            button_rects = show_menu(screen, font, selected_map_name)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if in_map_selector:
                    for rect, label in map_buttons:
                        if rect.collidepoint(event.pos):
                            selected_map_name = label
                            selected_map_file = map_options[label]
                            in_map_selector = False
                            break
                else:
                        ...
                        if rect.collidepoint(event.pos):
                            if action == "train":
                                if selected_map_file:
                                    print(f"Avvio training con: {selected_map_file}")
                                    env = VirtualEnvironment(48, 25, 32, screen,map_file=selected_map_file)
                                    # avvia il training
                                else:
                                    print("Seleziona una mappa prima di iniziare.")
                            elif action == "show":
                                pass  # logica per mostrare risultati
                            elif action == "select_map":
                                in_map_selector = True
                            elif action == "exit":
                                running = False
    pygame.quit()
